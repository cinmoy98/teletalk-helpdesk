...
class AbstractTicketForm(CustomFieldMixin, forms.Form):
    ...
    short_code = forms.CharField(
        ...
        widget=forms.TextInput(attrs={'class': 'form-control'}),
        label=_('Short Code'),
        help_text=_('Short code of the organization given in the job circular'),
    )
    ...
    org_name = forms.CharField(
        ...
        help_text=_('Organization name of the post you are applying for'),
    )
    ...
    post_name = forms.CharField(
        ...
        widget=forms.TextInput(attrs={'class': 'form-control'}),
        label=_('Post Name'),
        help_text=_('Name of the post you are applying for'),
    )
    ...
    def clean_sender_phone(self):
        data = self.cleaned_data['sender_phone']
        if len(data)==11 and (data.startswith("017") or data.startswith("015") or data.startswith("019") or data.startswith("013") or data.startswith("018") or data.startswith("014") or data.startswith("016")):
            return data
        else:
    ...
    def val_file_size(self):
        filesize = self.cleaned_data['attachment']
        filesize = filesize.size
        print("Entered validation###############################################################################")
        if filesize > 1048576:
            print("Entered validation if###############################################################################")
