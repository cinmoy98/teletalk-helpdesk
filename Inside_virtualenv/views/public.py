...
class BaseCreateTicketView(abstract_views.AbstractCreateTicketMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if text_is_spam(form.cleaned_data['body'], request):
            ...
            try:
                return HttpResponseRedirect('%s?ticket=%s&sender_phone=%s&key=%s' % (
                    reverse('helpdesk:public_view'),
                    ticket.ticket_for_url,
                    urlquote(ticket.sender_phone),
                    ticket.secret_key)
                )
            except ValueError:
...
class Homepage(CreateTicketView):
    template_name = 'helpdesk/kb_index.html'
...
def search_for_ticket(request, error_message=None):
    if hasattr(settings, 'HELPDESK_VIEW_A_TICKET_PUBLIC') and settings.HELPDESK_VIEW_A_TICKET_PUBLIC:
        sender_phone = request.GET.get('sender_phone', None)
...
def view_ticket(request):
    ...
    if not (ticket_req and sender_phone):
        if ticket_req is None and sender_phone is None:
            return search_for_ticket(request)
        else:
            return search_for_ticket(request, _('Missing ticket ID or Phone Number. Please try again.'))

    queue, ticket_id = Ticket.queue_and_id_from_query(ticket_req)
    try:
        if hasattr(settings, 'HELPDESK_VIEW_A_TICKET_PUBLIC') and settings.HELPDESK_VIEW_A_TICKET_PUBLIC:
            ticket = Ticket.objects.get(ticket_no=ticket_id, sender_phone=sender_phone)
        else:
            ticket = Ticket.objects.get(ticket_no=ticket_id, sender_phone=sender_phone, secret_key__iexact=key)
    except (ObjectDoesNotExist, ValueError):
        return search_for_ticket(request, _('Invalid ticket ID or Phone Number. Please try again.'))

    if is_helpdesk_staff(request.user):
        redirect_url = reverse('helpdesk:view', args=[ticket.id])
    ...
    if 'close' in request.GET and ticket.status == Ticket.RESOLVED_STATUS:
        ...
        return update_ticket(request, ticket.id, public=True)
